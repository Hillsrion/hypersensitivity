...
<template>
  ...
  >
    ...
    <button
      ...
      :class="[
        gameStore.isChoiceDisabled(choices[0])
          ? 'text-primary/30 cursor-not-allowed'
          : 'text-primary',
        !isSelecting && hoveredIndex !== null && hoveredIndex !== 0
          ? 'opacity-20'
          : 'opacity-100',
      ]"
      :disabled="gameStore.isChoiceDisabled(choices[0]) || isSelecting"
      @mouseenter="hoveredIndex = 0"
      @mouseleave="hoveredIndex = null"
      @click="handleSelect(choices[0], 0)"
    >
      ...
      <span
        v-if="gameStore.isChoiceDisabled(choices[0]) && choices[0].disabledReason"
        class="text-xs mt-1 text-primary/40 absolute top-full"
      >
        {{ choices[0].disabledReason }}
      </span>
    </button>
    ...
    <button
      ...
      class="group relative py-4 font-satoshi font-semibold text-xl/7 uppercase transition-all duration-300 flex flex-col items-center"
      :class="[
        gameStore.isChoiceDisabled(choices[1])
          ? 'text-primary/30 cursor-not-allowed'
          : 'text-primary',
        !isSelecting && hoveredIndex !== null && hoveredIndex !== 1
          ? 'opacity-20'
          : 'opacity-100',
      ]"
      :disabled="gameStore.isChoiceDisabled(choices[1]) || isSelecting"
      @mouseenter="hoveredIndex = 1"
      @mouseleave="hoveredIndex = null"
      @click="handleSelect(choices[1], 1)"
    >
      <span class="flex items-center gap-3">
        <span>{{ choices[1].text }}</span>
      </span>

      <!-- Raison de desactivation -->
      <span
        v-if="gameStore.isChoiceDisabled(choices[1]) && choices[1].disabledReason"
        class="text-xs mt-1 text-primary/40 absolute top-full"
      >
        {{ choices[1].disabledReason }}
      </span>
    </button>
  </div>
</template>
