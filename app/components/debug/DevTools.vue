...
onMounted(() => {
  ...
  const target =
    route.query.scroll || import.meta.env.VITE_START_POSITION || null
  if (target) {
...
<template>
  <div class="fixed bottom-4 right-4 z-99999 flex flex-col gap-2">
    ...
    >
      <div
        class="flex justify-between items-center mb-1 border-b border-white/20 pb-2"
      >
            ...
            <button
              ...
              :class="
                gameStore.forceShowUI
                  ? 'bg-orange-500/40 text-orange-200 outline-1 outline-orange-500'
        ...
        <template v-if="!gameStore.showQuestionnaire && showExperienceOptions">
          <div class="flex flex-col gap-1 px-2 py-1 bg-white/5 rounded">
            ...
            <select
              ...
              @change="
                (e: Event) => jumpToScene((e.target as HTMLSelectElement).value)
              "
            >
              <option v-for="id in SCENE_IDS" :key="id" :value="id">
                {{ getSceneLabel(id) }}
              </option>
            </select>
          </div>
