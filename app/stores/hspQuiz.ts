...
export const useHspQuizStore = defineStore('hspQuiz', {
  ...
  getters: {
    ...
    progressPercent(state): number {
      return Math.round(((state.currentQuestionIndex + 1) / this.totalQuestions) * 100);
    },
    
    displaySectionName(): string {
      return this.sections[this.currentSectionIndex]?.name || this.sections[0]?.name || '';
    },

    totalScore(state): number {
      return state.answers.reduce((sum: number, val: number | null) => sum + (val ?? 0), 0);
    },
    ...
    getSectionScore: (state) => (sectionIndex: number): number => {
      const start = sectionIndex * questionsPerSection;
      const end = start + questionsPerSection;
      return state.answers.slice(start, end).reduce((sum: number, val: number | null) => sum + (val ?? 0), 0) as number;
    },
